# -*- coding: utf-8 -*-
"""Quranic Translation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/163bGQRJj78hRU2U21t-yytuPx57-wqCb
"""
...
!git clone https://github.com/macbrennan90/translation-model.git

with open('/Quran-EN', "r") as f:
    data1 = f.read()
with open('/Quran-UR', "r") as f:
    data2 = f.read()
...
print("The longest Urdu sentence in our dataset is:", max_fr_length)
...
print('Number of unique Urdu words:', len(fr_word_count))
...
!wget https://dl.fbaipublicfiles.com/fasttext/vectors-wiki/wiki.ur.zip

!wget https://dl.fbaipublicfiles.com/fasttext/vectors-wiki/wiki.en.zip

!unzip /content/wiki.ur.zip -d /content

!unzip /content/wiki.en.zip -d /content

if os.path.exists('/content/en_words.npy') and os.path.exists('/content/en_vectors.npy'):
    en_words = np.load('/content/en_words.npy')
    en_vectors = np.load('/content/en_vectors.npy')
    ...
    en_vectors[0] = np.zeros(300) # make the padding vector zeros

    with open('/content/wiki.en.vec', "r") as f:
        f.readline()
    ...
    np.save('/content/en_words.npy', en_words)
    np.save('/content/en_vectors.npy', en_vectors)
...
if os.path.exists('/content/fr_words.npy') and os.path.exists('data/fr_vectors.npy'):
    fr_words = np.load('/content/fr_words.npy')
    fr_vectors = np.load('/content/fr_vectors.npy')
    print('Embeddings load from .npy file')
else:
    # make a dict with the top 100,000 words
    fr_words = ['<pad>',
    ...
    with open('/content/wiki.ur.vec', "r") as f:
        f.readline()
        for _ in range(100000):
            fr_vecs = f.readline()
    ...
    # Save the arrays so we don't have to load the full word embedding file
    np.save('/content/fr_words.npy', fr_words)
    np.save('/content/fr_vectors.npy', fr_vectors)

fr_word2idx = {word:index for index, word in enumerate(fr_words)}

chabeuil_idx = fr_word2idx['اور']
print('index for word اور:', chabeuil_idx, 
...
for i in range(5):
    print('Urdu Sentence:', batch['french_sentence'][i])
...
np.save('/content/lstm2_losses.npy', lstm_losses_cont)

np.save('/content/gru2_losses.npy', gru_losses)

lstm_losses = np.load('/content/lstm2_losses.npy')
gru_losses = np.load('/content/gru2_losses.npy')
...
torch.save(encoder_lstm.state_dict(), '/content/encoder2_lstm.pth')
torch.save(decoder_lstm.state_dict(), '/content/decoder2_lstm.pth')

torch.save(encoder_gru.state_dict(), '/content/encoder2_gru.pth')
torch.save(decoder_gru.state_dict(), '/content/decoder2_gru.pth')
