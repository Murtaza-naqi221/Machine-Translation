# -*- coding: utf-8 -*-
"""Neural_Machine_Translation_French_English

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/17JIDTs96OcgpOc2RM442e_kKUreRsu5U
...
with open(base_dir+'data/small_vocab_en.txt', "r") as f:
    data1 = f.read()
with open(base_dir+'data/small_vocab_fr.txt', "r") as f:
    data2 = f.read()
...
if os.path.exists(base_dir+'data/en_words.npy') and os.path.exists(base_dir+'data/en_vectors.npy'):
    en_words = np.load(base_dir+'data/en_words.npy')
    en_vectors = np.load(base_dir+'data/en_vectors.npy')
    ...
    with open(base_dir+'data/wiki.en.vec', "r") as f:
        f.readline()
    ...
    np.save(base_dir+'data/en_words.npy', en_words)
    np.save(base_dir+'data/en_vectors.npy', en_vectors)
...
if os.path.exists(base_dir+'data/fr_words.npy') and os.path.exists(base_dir+'data/fr_vectors.npy'):
    fr_words = np.load(base_dir+'data/fr_words.npy')
    fr_vectors = np.load(base_dir+'data/fr_vectors.npy')
    print('Embeddings load from .npy file')
else:
    # make a dict with the top 100,000 words
    fr_words = ['<pad>',
    ...
    with open(base_dir+'data/wiki.fr.vec', "r") as f:
        f.readline()
        for _ in range(100000):
            fr_vecs = f.readline()
    ...
    # Save the arrays so we don't have to load the full word embedding file
    np.save(base_dir+'data/fr_words.npy', fr_words)
    np.save(base_dir+'data/fr_vectors.npy', fr_vectors)
...
if not from_scratch:
    ...
    encoder_lstm_state_dict = torch.load(base_dir+'models/encoder1_lstm.pth')
    decoder_lstm_state_dict = torch.load(base_dir+'models/decoder1_lstm.pth')

    encoder_lstm.load_state_dict(encoder_lstm_state_dict)
    decoder_lstm.load_state_dict(decoder_lstm_state_dict)
    
        # Load weights from earlier model
    encoder_gru_state_dict = torch.load(base_dir+'models/encoder1_gru.pth')
    decoder_gru_state_dict = torch.load(base_dir+'models/decoder1_gru.pth')
...
np.save(base_dir+'data/lstm2_losses.npy', lstm_losses_cont)

np.save(base_dir+'data/gru2_losses.npy', gru_losses)

lstm_losses = np.load(base_dir+'data/lstm2_losses.npy')
gru_losses = np.load(base_dir+'data/gru2_losses.npy')
...
torch.save(encoder_lstm.state_dict(), base_dir+'models/encoder2_lstm.pth')
torch.save(decoder_lstm.state_dict(), base_dir+'models/decoder2_lstm.pth')

torch.save(encoder_gru.state_dict(), base_dir+'models/encoder2_gru.pth')
torch.save(decoder_gru.state_dict(), base_dir+'models/decoder2_gru.pth')
...
reference_sentence = batch['english_sentence'][11]+ " </s>"
ref = [
